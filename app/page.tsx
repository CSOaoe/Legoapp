    ...
    <section className="studio-workspace">
      ...
      <section className="forge-stage">
        ...
        <div className={`forge-canvas ${result ? "populated" : ""}`}>
          ...
          {generating ? <div className="forge-processing"><div className="forge-orbit"><Sparkles size={30} /></div><small>BRICKFORGE ENGINE</small><h2>{aiProgress ? aiProgress.split(" · ")[0] : "Analysing geometry"}</h2><div className="generation-track"><i style={{ width: `${generationProgress}%` }} /></div><span>{generationProgress}% · Keep this window open</span></div> : previewMesh && meshVolume ? <MeshConversionPreview mesh={previewMesh} volume={meshVolume} parts={result?.document.parts} /> : <div className="forge-empty"><div className="empty-emblem"><Blocks size={39} /></div><small>YOUR MODEL WILL APPEAR HERE</small><h2>{ready ? "Source loaded. Choose the build settings and generate." : "Bring any object into bricks."}</h2><span>Rotate and compare the reconstructed shape with its brick volume before opening the full editor.</span><button onClick={() => mode === "photos" ? photoInput.current?.click() : meshInput.current?.click()}>{mode === "photos" ? <ImagePlus size={16} /> : <FileUp size={16} />}{mode === "photos" ? "Choose images" : "Choose 3D file"}</button></div>}
          <div className="canvas-readout"><span><i />ORBIT VIEW</span><span>{sourceLabel}</span><span>INTERLOCKED PARTS</span></div>
        </div>
        {result ? <div className="build-metrics"><div><small>BRICKS</small><b>{result.document.parts.length.toLocaleString()}</b><span>{result.detailParts} sculpted surface parts</span></div><div><small>PART FAMILIES</small><b>{result.partFamilies.length}</b><span>{result.partFamilies.join(" · ")}</span></div><div><small>SHAPE COVERAGE</small><b>{result.shapeCoverage}%</b><span>{result.occupiedStuds.toLocaleString()} of {result.sourceStuds.toLocaleString()} source studs</span></div><div className={validationIssues.length ? "warning" : "valid"}><small>STRUCTURE</small><b>{validationIssues.length ? `${validationIssues.length} flags` : "Interlocked"}</b><span>{validationIssues.length ? "Open editor to review" : "connected to the base"}</span></div></div> : <div className="workflow-note"><span>01</span><b>Import source</b><i /><span>02</span><b>Shape the build</b><i /><span>03</span><b>Export instructions</b></div>}
      </section>
      ...
      <aside className="studio-rail refine-rail">
        ...
        <div className="construction-method"><Blocks size={18} /><span><b>M5 sculpted construction</b><small>{CATALOGUE.length} controlled parts across bricks, plates, slopes, curves, tiles and SNOT. No generated filler columns.</small></span></div>
