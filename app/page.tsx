    ...
    <section className="studio-workspace">
      ...
      <section className="forge-stage">
        ...
        <div className={`forge-canvas ${result ? "populated" : ""}`}>
          ...
          <div className="canvas-readout"><span><i />ORBIT VIEW</span><span>{sourceLabel}</span><span>{hollow ? "HOLLOW CORE" : "SOLID CORE"}</span></div>
        </div>
        {result ? <div className="build-metrics"><div><small>BRICKS</small><b>{result.document.parts.length.toLocaleString()}</b><span>{bom.length} unique groups</span></div><div><small>BUILD HEIGHT</small><b>{result.instructions.length}</b><span>ordered layers</span></div><div><small>SHAPE MATCH</small><b>{result.confidence}%</b><span>{meshVolume?.occupiedVoxels.toLocaleString()} occupied voxels</span></div><div className={validationIssues.length ? "warning" : "valid"}><small>STRUCTURE</small><b>{validationIssues.length ? `${validationIssues.length} flags` : "Connected"}</b><span>{validationIssues.length ? "Open editor to review" : "ready to build"}</span></div></div> : <div className="workflow-note"><span>01</span><b>Import source</b><i /><span>02</span><b>Shape the build</b><i /><span>03</span><b>Export instructions</b></div>}
      </section>
      ...
      <aside className="studio-rail refine-rail">
        ...
        <div className="construction-options"><label className="hollow-toggle"><input type="checkbox" checked={hollow} onChange={event => { setHollow(event.target.checked); invalidate(); }} /><span><b>Efficient hollow core</b><small>Preserves the outside shape with fewer bricks.</small></span></label><label className="hollow-toggle"><input type="checkbox" checked={supports} onChange={event => { setSupports(event.target.checked); invalidate(); }} /><span><b>Buildable supports</b><small>Adds hidden columns so every brick connects to the base.</small></span></label></div>
        <div className="build-estimate"><div><Sparkles size={17} /><span><small>CONVERSION ENGINE</small><b>{sourceLabel}</b></span></div><p>{photoEngine === "local-ai" && mode === "photos" ? "AI reconstructs the object first, then the brick engine creates a supported layered build." : "The source volume is sampled at brick scale and packed with real controlled part IDs."}</p></div>
