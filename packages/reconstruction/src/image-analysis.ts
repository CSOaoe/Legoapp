...
export async function analyseImage(file:File,samples=96):Promise<AnalysedImage>{
 ...
 const rows=Math.max(12,maxY-minY+1),topDown:number[]=[];for(let row=0;row<rows;row++){const sourceY=Math.round(minY+(row/(rows-1))*(maxY-minY));let left=width,right=-1;for(let offset=-1;offset<=1;offset++){const y=Math.max(0,Math.min(height-1,sourceY+offset));for(let x=0;x<width;x++)if(mask[y*width+x]){left=Math.min(left,x);right=Math.max(right,x)}}topDown.push(right>=left?(right-left+1)/Math.max(1,maxX-minX+1):0)}
 const widths=topDown.reverse().map((value,index,array)=>{const previous=array[Math.max(0,index-1)],next=array[Math.min(array.length-1,index+1)];return Math.max(0,Math.min(1,(previous+value*2+next)/4))});
 const confidence=Math.max(0,Math.min(1,(foreground/(width*height))/.35))*.55+.45;
 return{profile:{widths,confidence},preview:URL.createObjectURL(file),foregroundHex:`#${hex(r/foreground)}${hex(g/foreground)}${hex(b/foreground)}`,width:sourceWidth,height:sourceHeight};
}

export function mergeProfiles(profiles:SilhouetteProfile[]):SilhouetteProfile{
 if(!profiles.length)return{widths:[.5],confidence:0};const length=Math.max(...profiles.map(profile=>profile.widths.length)),widths=Array.from({length},(_,index)=>profiles.reduce((sum,profile)=>{const at=Math.round(index/(length-1)*Math.max(0,profile.widths.length-1));return sum+(profile.widths[at]??0)},0)/profiles.length);
 return{widths,confidence:profiles.reduce((sum,profile)=>sum+profile.confidence,0)/profiles.length};
}
